...
def evaluate(device, model, step, configs, logger=None, vocoder=None, losses=None):
    ...
    for batchs in loader:
        for batch in batchs:
            ...
            with torch.no_grad():
                # Forward
                output = model(*(batch[2:]), step=step,id=0)
    ...
    message = "Validation Step {}, Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Pitch Loss: {:.4f}, Energy Loss: {:.4f}, Duration Loss: {:.4f}, CTC Loss: {:.4f}, Binarization Loss: {:.4f}, Decoupling Loss Loss: {:.4f}".format(
